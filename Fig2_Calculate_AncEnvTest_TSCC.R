...
PathToAdmix <- "/projects/janssen/Ancestry/ANC_ENV_TEST/Group_Admix_Perc.txt"
...
############################################
## Group Admixture Estimate Summaries ######

## Load Admixture Estimates
Admix.1 <- read.table( PathToAdmix, sep="\t",header=T )
Admix <- Admix.1 / rowSums( Admix.1 )
rownames(Admix) <- paste( )
############################################
## Hi-Confidence Sites #####################
...
#############################################################
## CALCULATE CONTINGENCY STATS FOR EACH SET #################
#############################################################
...
## Calculate, for each chromosome & group:
 # Sensitivity = True_Pos / Cond_Pos
 # Specificity = True_Neg / Cond_Neg
 # Accuracy = ( True_Pos + True_Neg ) / Total
 # Positive Predictive Value = True_Pos / Test_Pos
...
## Test (Observed)
Test_Pos <- matrix( unlist(lapply( SNP, function(y) apply(y[,2:5],2,function(x) length(which(x!=0)) ) )), nrow=length(SNP),byrow=T )
Test_Neg <- HI_LENS - Test_Pos
colnames(Test_Pos) <- colnames(Test_Neg) <- names(SNP[[1]][2:5])
rownames(Test_Pos) <- rownames(Test_Neg) <- names(SNP)
## True/False Pos/Negs
True_Pos <- matrix( unlist(lapply( SNP, function(y) apply(y[,2:5],2,function(x) length(which(x!=0 & y[,1]!=0)) ) )), nrow=length(SNP),byrow=T )
Fals_Pos <- matrix( unlist(lapply( SNP, function(y) apply(y[,2:5],2,function(x) length(which(x!=0 & y[,1]==0)) ) )), nrow=length(SNP),byrow=T )
Fals_Neg <- matrix( unlist(lapply( SNP, function(y) apply(y[,2:5],2,function(x) length(which(x==0 & y[,1]!=0)) ) )), nrow=length(SNP),byrow=T )
True_Neg <- HI_LENS - ( True_Pos + Fals_Neg + Fals_Pos )
colnames(True_Pos) <- colnames(True_Neg) <- colnames(Fals_Pos) <- colnames(Fals_Neg) <- names(SNP[[1]][2:5])
rownames(True_Pos) <- rownames(True_Neg) <- rownames(Fals_Pos) <- rownames(Fals_Neg) <- names(SNP)
## Sens/Spec/Accur/PPV
Sens <- True_Pos / Cond_Pos
Spec <- True_Neg / Cond_Neg
Accu <- ( True_Pos + True_Neg ) / Total
PPV <- True_Pos / Test_Pos
NPV <- True_Neg / Test_Neg

## Compile Stats
COMPILE.1 <- list( Total, Cond_Pos, Cond_Neg, Test_Pos, Test_Neg )
names(COMPILE.1) <- c( "Total","Cond_P","Cond_N","Test_P","Test_N" )
COMPILE.2 <- list( True_Pos, Fals_Pos, True_Neg, Fals_Neg )
names(COMPILE.2) <- c( "True_P","False_P","True_N","False_N" )
COMPILE.3 <- list( Sens, Spec, Accu, PPV )
names(COMPILE.3) <- c( "Sensitivity","Specificity","Accuracy","PPV" )
COMPILE <- list( COMPILE.1, COMPILE.2, COMPILE.3 )
names(COMPILE) <- c( "Stat","TF","Calc" )
 # Save Results
save( COMPILE, file=paste(PathToSave,"Summary_Stats_",SI,"_",UF,".Rdata",sep="") )

## Compile into Handy Table
LIST <- list( Total, Cond_Pos, Cond_Neg, Test_Pos, Test_Neg, True_Pos, Fals_Pos, True_Neg, Fals_Neg, Sens, Spec, Accu, PPV )
names(LIST) <- c( "Total","Cond_P","Cond_N","Test_P","Test_N", "True_P","False_P","True_N","False_N", "Sensitivity","Specificity","Accuracy","PPV" )
GRP_TAGS <- colnames(SNP[[1]])[2:5]
FRAME <- data.frame( Admix, GRP=rep(GRP_TAGS[c(4,2,1,3)],rep(4,4)), CHR=rep(CHR_TAGS,4) )
FRAME <- FRAME[ order(FRAME$GRP), ]
FRAME <- data.frame( FRAME, Total, Cond_Pos, Cond_Neg )
for ( b in 4:length(LIST) ) {
	table <- LIST[[b]]
	FRAME <- data.frame( FRAME, c( table[,order(colnames(True_Pos))]) )
	colnames(FRAME)[ncol(FRAME)] <- names(LIST)[b]
}
 # Save Results
write.table( FRAME, file=paste(PathToSave,"Summary_Stats_",SI,"_",UF,".txt",sep=""),sep="\t",row.names=T,col.names=T,quote=F )
